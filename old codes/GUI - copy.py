...
class App(ctk.CTk):
    ...
    def update_frames(self):
        for cam_name, cap, camera_label, box_coords in (
            ...
            if ret and frame is not None:
                ...
                cv2.normalize(frame, frame, 0, 800, cv2.NORM_MINMAX)
                # cv2.imwrite(f"{cam_name}.png", frame)  # save frame
    ...
    def add_text(self, img, coords, hsv_values, colours):
        for i, (x, y) in enumerate(coords):
            ...
            # img = cv2.putText(img, f"{i}", (x-8, y+10), cv2.FONT_HERSHEY_SIMPLEX, 1, BOX_COLOUR, 4, 2)
        return img
